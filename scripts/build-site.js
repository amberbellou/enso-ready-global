...
for (const f of fs.readdirSync(path.join(ROOT, "src/vendor"))) fs.copyFileSync(path.join(ROOT, "src/vendor", f), path.join(SITE, "vendor", f));
fs.mkdirSync(path.join(SITE, ".well-known"), { recursive: true });
W(".well-known/security.txt", `Contact: https://github.com/amberbellou/enso-ready-global/security/advisories/new\nExpires: ${new Date(Date.now() + 365 * 86400000).toISOString()}\nPreferred-Languages: en, fr, es\nCanonical: https://amberbellou.github.io/enso-ready-global/.well-known/security.txt\nPolicy: https://github.com/amberbellou/enso-ready-global/blob/main/SECURITY.md\n`);
